...
class OrderController {
  static createOrder = async (req, res, next) => {
    ...
    try {
      ...
      await Request.findByIdAndUpdate(
        requestId,
        {
          requestStatus: 'Ordem de Serviço',
  ...
  static getAllOrdersOfficer = async (req, res, next) => {
    ...
    try {
      ...
      const officerId = existingOfficer._id;

      const ordersList = await Order.find({ officerId })
        .populate({
          path: 'requestId',
          select:
      ...
      if (ordersList.length === 0) {
        return next(
          new RequestError({
            title: 'Não há Ordens de Serviço!',
            msg: 'Nenhuma Ordem de Serviço Programada para o Técnico.',
          })
        );
      }
