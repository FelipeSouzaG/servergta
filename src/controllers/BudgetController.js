...
class BudgetController {
    ...
    try {
      ...
      await Request.findByIdAndUpdate(
        requestId,
        {
          ...
          feedback: `Orçamento (${newBudget.budgetNumber}) enviado para aprovação`,
  ...
  static updateBudget = async (req, res, next) => {
    ...
    try {
      ...
      res.status(200).json({
        ...
        msg: `Orçamento ${updatedBudget.budgetStatus} pela GTA.`,
        status: 200,
        level: level,
      });
    } catch (error) {
  ...
  static updateClientBudget = async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
      const { budgetId } = req.params;
      ...
      res.status(200).json({
        ...
        msg: `Orçamento ${updatedBudget.budgetStatus} pelo Cliente.`,
        status: 200,
        level: level,
      });
    } catch (error) {
